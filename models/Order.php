...
class Order {
     ...
     public static function getOrdersList()
    {
        ...
        $result = $db->query('SELECT id, user_id,user_name, user_phone, date, status FROM product_order ORDER BY id DESC');
        $ordersList = array();
        $i = 0;
    ...
    public static function getOrderByUserId($user_id)
    {
        ...
        // Текст запроса к БД
       $result = $db->query('SELECT id, user_id,user_name, user_phone, date, status FROM product_order WHERE user_id = '.$user_id.' ORDER BY id DESC');
        $ordersList = array();
        $i = 0;
        while ($row = $result->fetch()) {
            $ordersList[$i]['id'] = $row['id'];
